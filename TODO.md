...
Fixed 2026-09-24 (verified, no code change needed): a Form Model bound to a Combination Model can be the host of an include after all - `FormModelEditorController.resolveDataBindingDocumentModel` already resolves the combination to its merged stand-in `DocumentModel` via `ProjectDocumentModels.resolveDocumentModelForFieldReferences`/`getOtherDocumentModelsWithCombinations` (landed 2026-09-20 as part of the Query Model semantic filter validation work, for the Overview Model case), so `FormModelIncludeActions.canIncludeInto`'s `documentModel != null` check already passes and *Include Form Model...* is offered normally. There was no regression test for this combination specifically, so `FormModelActionsCombinationIncludeMenuTest` (using the `advanced_new` workspace's `PersonEmployee_Fm`/`PersonEmployee_Cm`) pins it.

Fixed 2026-09-24: the Model Settings dialog for an Additive Document Model now matches SME's `AdmSettingsEditorView` layout - Supported Characters, Model References, Timezone, Model Configuration and Model Info are hidden (they don't apply: those live on the reference model, or - for Timezone/Model Config - aren't meaningful for an overlay), and the raw Annotations panel is split into a read-only "Reference Model Annotations" panel (the resolved base model's own header annotations, via `AdditiveDocumentModels.findBaseModel`, hidden if no Combination Model resolves one yet) followed by an editable "Additive Model Annotations" panel (the Additive Document Model's own header annotations, unchanged otherwise) - `ModelSettingsDialog.initialize()`, new `reference-model-annotations-panel.fxml` sharing `AnnotationsPanelController` (which gained public `setVisible`/`setTitle` for this) with `annotations-panel.fxml`. The `instanceof DocumentModel`-filter/heterogeneity-annotation-hiding note above was already correct and needed no fix.
