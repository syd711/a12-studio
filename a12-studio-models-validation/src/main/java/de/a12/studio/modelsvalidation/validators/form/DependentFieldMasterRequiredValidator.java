...
/** A {@code dependentField} block requires a master field to be selected. */
public final class DependentFieldMasterRequiredValidator implements ModelValidator {
  ...
  public List<ModelValidationError> validate(A12Model<?> model, ValidationContext context) {
    ...
    for (FieldConfigEntry entry : formModel.getContent().getFieldConfiguration().getField()) {
      if (entry.getDependentField() != null
          && (entry.getDependentField().getMasterField() == null || entry.getDependentField().getMasterField().isBlank())) {
        errors.add(new ModelValidationError(model, FormFieldReferenceValidator.ELEMENT_ID,
            ValidationMessages.get("validation.dependentField.masterRequired", entry.getElementRef()),
            Severity.ERROR.name()));
      }
    }
    return errors;
