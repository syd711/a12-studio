    ...
    for (TreeNode node : treeModel.getContent().getNodes()) {
      if (node.getId() != null && !seenIds.add(node.getId())) {
        errors.add(new ModelValidationError(model, ELEMENT_ID,
            ValidationMessages.get("validation.treeUniqueNode.duplicateId", node.getId()), Severity.ERROR.name()));
      }
      if (node.getDocumentModelRef() != null && !node.getDocumentModelRef().isBlank()
          && !seenDocumentModels.add(node.getDocumentModelRef())) {
        errors.add(new ModelValidationError(model, ELEMENT_ID,
            ValidationMessages.get("validation.treeUniqueNode.duplicateDocumentModel", node.getDocumentModelRef()),
            Severity.ERROR.name()));
      }
    }
    return errors;
