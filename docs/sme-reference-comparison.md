...
| Move/rename refactoring (rewrite condition text) | `MoveRefactoringService` | Present in-process for one model (clean-room `DocumentModelRefactoring`, no kernel call); cross-model references still missing (see Document Model gap list above) |
